<template>
    <FormItem :prop=rule>
            ...
            <div class="multi-upload-list">
                <div class="upload-list" v-for="item in uploadList" :key="item.index">
                    <template v-if="item.status == 'finished'">
                        <img v-if="item.response" :src="item.response" @click="handleView(item.response)">
                        <div class="upload-control" @click="handleRemove(item)" v-show="meta && meta.disabled ? false : true">Устгах</div>
                    </template>
        ...
        <Upload
            ...
            :disabled="meta && meta.disabled ? meta.disabled : false">
            <Button type="dashed">
                <img class="preview-img" v-if="this.model.form[this.model.component] != null" :src="this.model.form[this.model.component]"
                     alt="image">
                <div>
                    <i class="ti ti-camera"></i>
                    {{ label }}
        ...
        <Modal title="Зураг харах" v-model="showImage" width="1000px">
            <img :src="showImageUrl" v-if="showImage" style="width: 100%">
        </Modal>
    </FormItem>
</template>
